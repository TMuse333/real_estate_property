...
<motion.h1
        ...
        {!isDesktop ? (
            ...
            //  animate={inView? imgVariants.animate : null}
              src={profile.image || tom} className='profile-image' />
            <motion.h2
            // initial={textVariants(true,2).initial}
            // animate={tiltFinished ? textVariants(true,2).animate : null}
            >
               {profile.name || 'Your name Here'}</motion.h2>
            <h3>{profile.company || 'Company Name'}</h3>
            <p>{profile.title || 'Your position here'}</p>
            {/* <p>REL#1234567890</p> */}
...
<motion.p
 ...
 >{profile.number || '123 456 794'}</motion.p>
<motion.p
 initial={textVariants(false,2).hidden}
 animate={tiltFinished ? textVariants(false,0.6).animate : null}
>{profile.email || 'Your email here'}</motion.p>
            ...
            <motion.p
              initial={textVariants(true,2).hidden}
              animate={tiltFinished ? textVariants(true,0).animate : null}
            >{profile.title || 'Your position here'}</motion.p>
            <motion.p
              initial={textVariants(true,2).hidden}
           ...
           src={profile.image || tom}  className='profile-image' />
              ...
              <div className="profile-contacts">
                    ...
                    <motion.p
                     initial={textVariants(false,2).hidden}
                     animate={tiltFinished ? textVariants(false,0.6).animate : null}
                     >{profile.number || '123 456 794'}
                     </motion.p>
                    <motion.p
                     initial={textVariants(false,2).hidden}
                     animate={tiltFinished ? textVariants(false,0.6).animate : null}
                    >T{profile.email || 'Your email here'}</motion.p>
