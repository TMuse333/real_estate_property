...
const {profileImage, setProfileImage,setCreatePageClicked,} = useImageContext()
...
<motion.h1 className="profile-h1"
        ...
        {!isDesktop ? (
            ...
            >
               {name || 'Your name Here'}</motion.h2>
            <h3>{companyName || 'Company Name'}</h3>
            <p>{position || 'Your position here'}</p>
...
<motion.p
 ...
 >{phoneNumber || '123 456 794'}</motion.p>
<motion.p
 initial={textVariants(false,2).hidden}
 animate={tiltFinished ? textVariants(false,0.6).animate : null}
>{profileEmail || 'Your email here'}</motion.p>
            ...
            <motion.p
              initial={textVariants(true,2).hidden}
              animate={tiltFinished ? textVariants(true,0).animate : null}
            >{position || 'Your position here'}</motion.p>
            <motion.p
              initial={textVariants(true,2).hidden}
