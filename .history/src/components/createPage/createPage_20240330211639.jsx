...
const CreateText = ({ onSave }) => {
  ...
  const handleChange = (e, featureKey) => {
    ...
    if (featureKey) {
      ...
      console.log('feature key is true',e.target.value)
    } else {
  ...
  const handleSaveIndividual = (key,e) => {
    ...
    console.log('property data uploaded',propertyData)
  };
        ...
        {featureInputs.map((feature, index) => (
          <div key={index} style={{ display: index === currentIndex ? 'block' : 'none' }}>
            {feature.image ? (
              ...
              <button onClick={() => handleSaveIndividual(feature.key,e)}>Save {feature.name}</button>
          </div>
