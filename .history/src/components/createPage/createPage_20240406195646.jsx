...
const CreateText = ({ onSave }) => {
  ...
  const {sliderImages, setSliderImages} = useImageContext(numOfCarousels)
  const {createPageClicked, setCreatePageClicked} = useImageContext()
...
useEffect(()=>{
  console.log('the sloider list is',sliderImages)

},[sliderImages])

useEffect(()=>{
  console.log('herobanner is:',propertyHerobanner)
},[propertyHerobanner])
        ...
        {featureInputs.map((feature, index) => (
          <div key={index} style={{ display: index === currentIndex ? 'block' : 'none' }}>
            {feature.image ? (
              ...
              multiple={feature.multipleImages}
              imageArray={feature.imageArray} />
            ) : 
            feature.isProfile ? (
           <Profile
           inputVariant={true}
